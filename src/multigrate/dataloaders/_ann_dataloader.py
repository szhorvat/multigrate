...
class StratifiedSampler(torch.utils.data.sampler.Sampler):
    ...
    :param group_labels:
        list of group values, same shape as 'indices'
    :param batch_size:
...
class GroupAnnDataLoader(DataLoader):
    ...
    :param group_column:
        Column name in `.obs` on which to stratify.
    :param shuffle:
...
# https://github.com/YosefLab/scvi-tools/blob/ac0c3e04fcc2772fdcf7de4de819db3af9465b6b/scvi/dataloaders/_ann_dataloader.py#L15
# accessed on 4 November 2021
class StratifiedGroupAwareSampler(torch.utils.data.sampler.Sampler):
    """Custom stratified sampler class which enables sampling the same number of observation from each group in each mini-batch, preserving celltype frequencies in each group.

    :param indices:
        list of indices to sample from
    :param group_labels:
        list of group values, same shape as 'indices'
    :param celltype_labels:
        list of celltypes, same shape as 'indices'
    :param batch_size:
        batch size of each iteration
    :param shuffle:
        if ``True``, shuffles indices before sampling
